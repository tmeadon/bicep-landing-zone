...
// if the resource references a key vault then find that key vault resource config
var keyVault = contains(resourceConfig.properties, 'keyVaultRef') ? resources[resourceConfig.properties.keyVaultRef] : null

// generate a unique suffix for the resource
var uniqueSuffix = '-${take(uniqueString(resourceGroup().id), 5)}'

// if a key vault is referenced then generate a unique suffix using it's resource group ID and then append it to the
// name generated by the naming module
var refKvResourceGroupId = keyVault != null ? subscriptionResourceId('Microsoft.Resources/resourceGroups', keyVault.resourceGroup) : ''
var refKvSuffix = keyVault != null ? (keyVault.properties.appendUniqueString ? '-${take(uniqueString(refKvResourceGroupId), 5)}' : '') : ''
var refKvName = keyVault != null ? '${keyVaultNaming.outputs.keyVault}${refKvSuffix}' : ''
...
module vault 'kv.bicep' = if (resourceConfig.type == 'keyVault') {
  ...
  params: {
    name: resourceConfig.properties.appendUniqueString ? '${naming.outputs.keyVault}${uniqueSuffix}' : naming.outputs.keyVault
    location: resourceConfig.location
